...
impl XBuffer {
    ...
    #[inline(always)]
    pub fn get(&self, i: usize) -> XBufferValue {
        XBufferValue {
            score: self.score[i],
            from: self.from[i] << 1 >> 1,
            flip_strand: (self.from[i] >> 31) == 1,
        }
    }
    ...
    pub fn fill<F: MatchFunc>(
        ...
        // 1. suffix_score[k] = max(suffix_score[k + 1], S[cur][k] + jump_penalty)
        // 2. prefix_score[k] = max(prefix_score[k - 1], S[cur][k] + jump_penalty)
        ...
        // base case
        self.score[m] = S[cur][m] + scoring.jump_score;
        self.from[m] = m as u32;
        // iterate over k in descending order
        for k in (0..m).rev() {
            let k_score = S[cur][k] + scoring.jump_score;
        ...
        // base case
        let mut prev_prefix_score = S[cur][0] + scoring.jump_score;
        let mut prev_prefix_from = 0u32;
        // iterate over k in ascending order
        for k in 1..=m {
            // compute prefix_score[k]
            let cur_prefix_score = S[cur][k] + scoring.jump_score;
